...
readData<-function(file){
  ...
  data$V2<-trimws(data$V2,which = "both")
  ...
  list.save(th, paste0("th_",format(Sys.time(),"%y%m%d%H"),'.json')) ##save json
  for (i in 1:length(th)){
    p<-ggplot(th[[i]],aes(x=factor(rep(1:(nrow(th[[i]])/2),2)),y=value,group=variable,col=variable))+geom_line()+facet_grid(.~variable)+ggtitle(paste0("(",format(Sys.time(),"%y/%m/%d/%Hh"),")Bus index: ",names(th[i]))) + 
      theme(plot.title = element_text(lineheight=.8, face="bold"),axis.text.x=element_blank())+xlab("Time")
    fun_plot(p,plotname = paste0("th_",format(Sys.time(),"%y%m%d%H"),".png"))
  }
...
file<-"c:/LOG/busInformation_idx506-01-04.txt"
readData(file)
