...
#2*3 차원의 가중치
weight<-matrix(1:6,nrow=2,ncol=3)
#1*3 차원의 오차항
bias<-matrix(1:3,nrow=1,ncol=3)
#입력값*가중치+오차항
input%*%weight+bias
#차원이 맞지 않아 계산 안 됨 -> sweep함수 사용 
result<-sweep(input%*%weight,2,bias,'+')
...
predict.dnn<-function(model, data=X.test){
  ...
  #sweep을 이용하여 hidden layer의 차원을 맞춰줌
  hidden.layer<-sweep(new.data%*%model$W1,2,model$b1,'+')
  
  #hidden layer 값 중 최대치를 골라서 score(예측값)을 찾는데 사용
...
train.dnn<-function(x,y,traindata=data,testdata=NULL,
                    #hidden layer과 변수의 갯수를 고름(아래는 1*6으로 지정)
                    hidden=c(6),
                    #최대 반복 수치 지정(클수록 속도는 줄어들고 정확성은 올라감)
                    maxit=2000,
                    #delta loss(반복할 때마다 약간씩 수정하는 값의 크기)
                    abstol=1e-2,
                    #learing rate(학습 속도, 작을수록 느리지만 대신 정확함)
                    lr=1e-2,
                    #regularization rate(추가적인 feature를 도입하는 속도, 각 fearute간 cross-product, 제곱 등이 쓰인다.)
                    reg=1e-3,
                    #총 100번의 결과값을 보여준다.
                    display=100,
                    random.seed=1){
  ...
  #input값들
  D<-ncol(X)
  #classfication이 적용되는 값의 종류(ex.0,1)
  K<-length(unique(Y))#unique(Y)값이 100개라면 총 100개의 가능성으로 결과값을 보여준다(사과, 배, 수박..이런식)
  H<-hidden
  #최초 시작점을 정한다.(local max를 찾을 때 최초 시작점이 매우 중요)
  W1<-0.01*matrix(rnorm(D*H),nrow=D,ncol=H)
  b1<-matrix(0,nrow=1,ncol=H)
  W2<-0.01*matrix(rnorm(H*K),nrow=H,ncol=K)
  b2<-matrix(0,nrow=1,ncol=K)
  #Traing데이터 전체를 training에 활용(N을 그룹으로 쪼개서 bagging을 할 수도 있음)
  batchsize<-N
  loss<-100000
  #trianing데이터를 NN에 적용
  i<-0
  while(i<maxit&&loss>abstol){
    #iteration index
    i<-i+1
    #forward로 계산(아래에 backward로 계산도 있음)
    #1은 행, 2는 열
    ...
    #softmax(loss function을 지정.cross-entropy라고도 함)
    score.exp<-exp(score)
    probs<-sweep(score.exp,1,rowSums(score.exp),'/')
    #Loss 값을 찾는다 - 매우 작은 값이 나와야 모델이 제대로 "학습"한 것임
...
head(iris)
plot(iris)
###1. Testset과 Trainingset으로 데이터 분리
